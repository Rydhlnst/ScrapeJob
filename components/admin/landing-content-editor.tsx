...
export function LandingContentEditor({ initialRecord }: { initialRecord: AdminLandingPageContentRecord }) {
  ...
  return (
    <div className="space-y-6">
      ...
      <Tabs defaultValue={activeTab} orientation="vertical" className="flex-col gap-6 lg:flex-row">
        <aside className="w-full lg:w-[280px]">
          <TabsList className="h-auto w-full flex-col items-stretch rounded-none border border-border bg-white p-0 shadow-[var(--shadow-sm)]" variant="line">
            {sections.map((section) => (
              <TabsTrigger key={section.value} value={section.value} className="h-auto w-full justify-start rounded-none border-b border-border px-4 py-3 text-left data-active:bg-[var(--brand-shell)]">
                <span className="flex flex-col items-start gap-1">
                  <span className="text-xs font-semibold uppercase tracking-[0.14em] text-[var(--brand-ink)]">{section.label}</span>
                  <span className="text-xs normal-case tracking-normal text-slate-500">{section.description}</span>
                </span>
              </TabsTrigger>
            ))}
          </TabsList>
