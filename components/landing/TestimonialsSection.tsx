    ...
    <section className="relative overflow-hidden bg-white py-20 md:py-28" id="about">
      <Container>
        ...
        <div className="mx-auto mt-14 grid max-w-5xl gap-5 lg:grid-cols-[.72fr_1.28fr_.72fr] lg:items-center">
          <article className="rounded-[24px] border border-black/10 bg-[#f6f9fc] p-6 text-left"><p className="text-sm leading-6 text-slate-600">“{supporting[0]?.quote}”</p><p className="mt-5 text-sm font-semibold text-[#171717]">{supporting[0]?.name}</p></article>
          <article className="order-first rounded-[30px] border border-black/10 bg-white p-8 text-center shadow-[0_10px_0_rgba(23,23,23,.05)] lg:order-none">
            ...
            <p className="mt-6 text-sm font-semibold text-[#171717]">{featured?.name}</p>
            <p className="mt-1 text-xs text-slate-500">{content.featuredRole}</p>
          </article>
          <article className="rounded-[24px] border border-black/10 bg-[#f6f9fc] p-6 text-left"><p className="text-sm leading-6 text-slate-600">“{supporting[1]?.quote}”</p><p className="mt-5 text-sm font-semibold text-[#171717]">{supporting[1]?.name}</p></article>
        </div>
      </Container>
    </section>
  )
